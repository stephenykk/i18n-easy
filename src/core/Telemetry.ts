...
export class Telemetry {
  ...
  static get userId() {
    if (this._id)
      return this._id
    this._id = Config.ctx.globalState.get('i18n-easy.telemetry-user-id')!
    if (!this._id) {
      this._id = uuid()
      Config.ctx.globalState.update('i18n-easy.telemetry-user-id', this._id)
    }
  ...
  static checkVersionChange() {
    const previousVersion = Config.ctx.globalState.get('i18n-easy.previous-version')
    ...
    Config.ctx.globalState.update('i18n-easy.previous-version', version)
  }
